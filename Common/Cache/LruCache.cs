...
public class LruCache<TK, TV>(int capacity) where TK : notnull
{
    ...
    {
        if (!_cache.TryGetValue(key, out var node)) throw new KeyNotFoundException("Key not found in cache.");
    ...
    {
        foreach (var item in _order) Console.Write($"[{item.Key}:{item.Value}] ");
